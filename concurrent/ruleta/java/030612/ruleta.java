			...
			switch (Character.toLowerCase(opc) ) {
				...
				case '-':
				 if (JUG.size() > 1) {
				   ...
				   // Tenim un vector amb el identificadors del jugadors morts...
				   // Els jugadors se encarregaran d mirar si els han mort o
				   // la ruleta a finalitzat... 
				   regioCritica.seccEnt();
				   regioCritica.mortJug.add(new Integer(JUG.size()));
				   regioCritica.seccSal();
				 } else {
			           System.out.println("Com a minim hem de tenir 1 jugador :P");
				 }
				 break;
				case 's': 
				   continuar = false;
