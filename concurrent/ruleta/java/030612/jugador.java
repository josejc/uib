   ...
   */
   public void run () {
      ...
      try {
	    ...
	    // Aquest bucle es repeteix fins q hagin mort al jugador.... O...
	    regioCritica.seccEnt();
	    if (!regioCritica.mortJug.isEmpty()) {
	       for (Enumeration e = regioCritica.aceptades.elements(); e.hasMoreElements(); ){
		  mort = (Integer)e.nextElement();
	          if (mort.intValue() == idJugador) {
		     regioCritica.mortJug.remove(mort);
		     regioCritica.seccSal(); // Aquest jugador ha de morir i sortim abans ;)
		     finalitzar();
		  }
	       }
	    } 	    
	    ...
	    regioCritica.seccEnt();
	      ...
	      System.out.println("El jugador "+idJugador+" te pendent...");
	      ap.escriure();
	    regioCritica.seccSal();
